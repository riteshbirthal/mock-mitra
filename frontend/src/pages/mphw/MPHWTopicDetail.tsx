...
const MPHWTopicDetail = () => {
  ...
  useEffect(() => {
    const fetchTopicData = async () => {
      ...
      try {
        // Fetch from the backend content folder
        const response = await fetch(`/api/mphw/content/${subjectId}/${topicId}`);
        if (!response.ok) {
          // Try loading static JSON file
          const staticResponse = await fetch(`/content/MPHW/${subjectId.charAt(0).toUpperCase() + subjectId.slice(1)}/${topicId}.json`);
        ...
        setTopicData(getMockTopicData(topicId || ''));
      } finally {
        setLoading(false);
      }
    };
  ...
  const getMockTopicData = (id: string): TopicData => {
    return {
      topic: id.replace(/-/g, ' ').replace(/^\d+\s*/, '').replace(/\b\w/g, l => l.toUpperCase()),
      subject: 'Reasoning',
      exam: 'HSSC MPHW',
  ...
  if (error || !topicData) {
    return (
      <div className="container">
        ...
        <Link to={`/mphw/topics/${subjectId}`} className="btn btn-primary" style={{ marginTop: '20px' }}>
          Back to Topics
        </Link>
      </div>
    );
  ...
  return (
    <div className="mphw-topic-detail-page">
      <div className="container">
        {/* Breadcrumb */}
        <nav className="breadcrumb">
          <Link to="/mphw">MPHW Prep</Link>
          <span>/</span>
          <Link to={`/mphw/topics/${subjectId}`}>{topicData.subject}</Link>
          <span>/</span>
            ...
            <div className="theory-section">
              ...
              {topicData.theory.types && (
                <div className="theory-card">
                  ...
                  {topicData.theory.types.map((type: any, index: number) => (
            ...
            <div className="practice-section">
              ...
              <div className="question-navigation">
                ...
                {currentQuestion < topicData.questions.length - 1 ? (
                  ...
                  <Link to={`/mphw/topics/${subjectId}`} className="btn btn-success">
                    Complete ✓
                  </Link>
                )}
              </div>
        ...
        <div className="bottom-nav">
          <Link to={`/mphw/topics/${subjectId}`} className="btn btn-secondary">
            ← Back to Topics
          </Link>
...
export default MPHWTopicDetail;
